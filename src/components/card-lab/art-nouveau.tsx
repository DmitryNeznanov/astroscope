 ...
 * Default: The Hermit (IX) — robed figure in profile on a mountain peak,
 * lantern aloft, staff in hand. Flowing bezier line work, halo disk with
 * radiating pattern, whiplash curves, botanical border, medallion numeral,
 * ribbon title. Reusable via { number, name, variant } props; variant 0 and
 * no props render the original Hermit card exactly.
...
export interface ArtNouveauCardProps {
  /** Major arcana number 1-22 → roman numeral in the top medallion. */
  number?: number;
  /** Card name letterspaced on the ribbon (auto-fitted when long). */
  name?: string;
...
function IrisDef({ pal }: { pal: NouveauPalette }) {
  return (
    <g id="cl-an-iris">
...
export default function ArtNouveauHermitCard({
  ...
  return (
    <figure
      ...
      role="img"
      aria-label={`${name} tarot card in Art Nouveau style`}
