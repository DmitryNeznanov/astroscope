...
export default function OrreryLanding() {
  return (
    <div className="lo-root min-h-screen">
      <style>{`
        ...
        .lo-plate:hover { border-color: rgba(232,200,122,.7); box-shadow: 0 0 24px -6px rgba(201,162,39,.3), inset 0 0 0 2px #0d0a06, inset 0 0 0 3px rgba(232,200,122,.3); transform: translateY(-2px); }
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 lg:py-20">
        ...
        <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {SIGNS.map((s) => (
            <a key={s.n} href={`/horoscope/${s.n.toLowerCase()}`} className="lo-plate group flex flex-col items-center px-3 py-5 text-center">
      ...
      <section className="mx-auto max-w-6xl px-6 py-10 lg:py-16">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {PANELS.map((p) => (
            <article key={p.title} className="lo-panel relative p-7">
              {RIVET_POS.map((pos) => (
                <span key={pos} className={`lo-rivet ${pos}`} aria-hidden="true" />
              ))}
              <div className="lo-mech mx-auto flex h-[120px] items-center justify-center">{p.mech}</div>
              <div className="mt-6 flex items-baseline justify-between gap-3">
                <h3 className="lo-serif text-xl text-[#f0e9d8]">{p.title}</h3>
